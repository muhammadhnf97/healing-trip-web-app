...
const Sidebar = ({ isSideBar, handleClickIsSideBar, handleClickLogout, loginData, pinPoint }) => {
  return (
    <>
    <button className={`${isSideBar ? 'visible bg-opacity-50' : 'invisible bg-opacity-0'} duration-100 fixed w-screen h-screen right-0 bg-black z-10 `} onClick={handleClickIsSideBar}></button>
    <div className={`${isSideBar ? 'visible translate-y-16' : 'invisible -translate-y-0' } ease-out duration-200 bg-white fixed right-0 -top-10 w-full h-fit z-10 px-10 pb-5 font-semibold text-center text-lg shadow-lg font-poppins md:hidden `}>
      <button className='absolute -top-2 right-5' onClick={handleClickIsSideBar}><RxCross1 /></button>
        {
